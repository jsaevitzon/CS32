...
bool DiskMultiMap::createNew(const std::string& filename, unsigned int numBuckets)
{
    ...
    {
        cerr<<"Something went wrong"; //TODO
        return false;
    }
...
{
    if(bf.isOpen()) //TODO: coould maybe even get rid of this line
        bf.close();
}
...
int DiskMultiMap::erase(const std::string& key, const std::string& value, const std::string& context)
{
    ...
    //while we are pointing to a node, this loop shouldnt run more than once
...
{
    //TODO: may have to change this to figure out how to hold erased items
    ...
    //array of Binary File Offsets
    //BinaryFile::Offset openHash[numBuckets];
    
    //TODO: DECIDE
    //This may work, but I can also try and use an array
    
    
    BinaryFile::Offset startOfHash = bf.fileLength();
    for(int j = 0; j<numBuckets; j++)
    {
        bf.write(pointToNegative, startOfHash);              //Point every bucket to -1 to start
...
//Searching for the parent node of a particular key
//Returns the location of the first node that the parent node is pointing to
//If it is not found, it returns the offset of the last parent node
BinaryFile::Offset DiskMultiMap::searchThroughTree(std::string key, bool &found, BinaryFile::Offset &locParent)
{
    ...
    for(;;)
    {
        ...
        // if(locFirstNode==pointToNegative)
        // return pointToNegative;
